...
void AIAmSadCharacter::BeginPlay()
{
	...
	if (HealthComponent)
	{
		HealthComponent->OnDeath.AddDynamic(this, &AIAmSadCharacter::HandleDeath);
	}
}
...
void AIAmSadCharacter::ApplyFallDamage(float FallSpeed)
{
	...
	if (HealthComponent)
	{
		...
		// Check if this killed us (death sound/restart handled by HandleDeath via delegate)
		if (HealthComponent->CurrentHealth <= 0.0f)
		{
		...
		{
			// Play fall damage sound (different from regular damage sound)
			if (FallDamageSound)
			{
...
void AIAmSadCharacter::HandleDeath()
{
